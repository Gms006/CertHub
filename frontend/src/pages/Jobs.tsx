...
const JobsPage = () => {
  ...
  const lastUpdated = useMemo(() => {
    const timestamps = jobs.map((job) => job.updated_at).filter(Boolean);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl bg-white/70 p-4 shadow-sm ring-1 ring-slate-200/70 backdrop-blur">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            className="flex h-10 items-center gap-2 rounded-2xl border border-slate-200/70 bg-white/70 px-4 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-white"
      ...
      <div className="flex items-center justify-between text-xs text-slate-500">
        <span>{filteredJobs.length} solicitações encontradas</span>
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right">
                    <details className="relative inline-block text-left">
                      <summary className="list-none rounded-full border border-slate-200/70 bg-white/70 px-2 py-1 text-sm text-slate-600 shadow-sm transition hover:border-slate-300 hover:text-slate-800">
                        ⋯
                      </summary>
                      <div className="absolute right-0 z-10 mt-2 w-40 rounded-2xl border border-slate-200/70 bg-white/95 p-2 text-xs text-slate-600 shadow-lg">
                        {job.status === "FAILED" ? (
